...
FW4_ORIG="LIBC_FATAL_STDERR_=1 /usr/local/Kobo/nickel -platform kobo -skipFontLoad \&"
FW4_NEW="LD_PRELOAD=/root/screenInv.so LIBC_FATAL_STDERR_=1 /usr/local/Kobo/nickel -platform kobo -skipFontLoad \&"

sed -i "s;^$FW1_ORIG;$FW1_NEW;" /etc/init.d/rcS
sed -i "s;^$FW2_ORIG;$FW2_NEW;" /etc/init.d/rcS
sed -i "s;^$FW3_ORIG;$FW3_NEW;" /etc/init.d/rcS
sed -i "s;^$FW4_ORIG;$FW4_NEW;" /etc/init.d/rcS
